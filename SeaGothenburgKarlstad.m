function [Energy, TotalTime, NumberOfBatteries,Cost] = SeaGothenburgKarlstad(s)
...
C_batt = 300000*3600;           % Capacity of a 40" battery container
NumberOfBatteries = ceil(Energy / C_batt);     % Number of 40" battery container required to do the trip
Cost = NumberOfBatteries * 400000;    % Battery cost in [€]
% fprintf('Required number of 40" battery containers to do the trip is: %d which in total costs: %.1f Million Euros \n', NumberOfBatteries,Cost/10^6);

%% Chargers capability
T_charger = 12 * 3600;          % Turn around time at port
P_charger = Energy / T_charger;
